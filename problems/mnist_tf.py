...
class MNISTClassificationProblem(BaseProblem):
    ...
    def __init__(self, val_split=0.2):
        ...
        # Flatten to 784-dim vectors and normalize to [0, 1]
        train_images = train_images.reshape(-1, 784).astype(np.float32) / 255.0

        # One-hot encode labels
        labels_onehot = np.eye(10)[train_labels.astype(int)]

        # Split into training and validation sets
        X_train, X_val, Y_train, Y_val = train_test_split(
            train_images, labels_onehot, test_size=val_split, random_state=42
        )
